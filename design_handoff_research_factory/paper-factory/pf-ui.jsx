...
const PF_ORDER = { generating: 0, awaiting_review: 1, queued: 2, approved: 3, rejected: 4, ingested: 5 };

function pfStatus(s) { return PF_STATUS[s] || PF_STATUS.ingested; }

function pfAuthors(list) {
...
function StatusChip({ status, retry }) {
...
function ScoreBar({ score, reproducible }) {
...
function CodeBlock({ code, height }) {
  const src = typeof code === "string" ? code : "";
  const lines = React.useMemo(() => (window.PFHighlight ? PFHighlight.lines(src) : src.split("\n").map(l => [{ c: "", t: l }])), [src]);
  if (!src.trim()) {
    return (
...
function Terminal({ output, label, running, maxHeight }) {
...
Object.assign(window, { PF_STATUS, PF_ORDER, pfStatus, pfAuthors, pfAgo, StatusChip, ScoreBar, CodeBlock, Terminal, Notice, SectionHead, pfOutClass });
