/* Research Factory - bundled snapshot fixture.
   Converted from the design handoff's pf-data.js (IIFE -> ES module).
   Serves two jobs: demo data when no API is configured, and the
   last-known snapshot when a live fetch fails, so a backend outage
   degrades to a populated screen instead of a blank one. */
